...
public class DownloadDataDao {
	...
	public List<Object[]> getalldata(String cur_date) {
		String sqlString = "select ud.id, ud.link, ud.domain_name,ud.source_link, ud.project_id, pro.project_name, pro.channel_name, pt.project_type, acc.account_name, pro.project_url, au.USER_NAME, ud.upload_time, ud.note1, ud.note2, ud.note3, ud.is_whitelist, ud.is_greylist, ud.link_type from upload_data ud, app_user au, project pro, project_types pt, account acc where ud.user_id = au.USER_ID and ud.project_id = pro.id and pro.project_types_id = pt.id and pro.account_id = acc.id and date(ud.upload_time)=:cur_date";
